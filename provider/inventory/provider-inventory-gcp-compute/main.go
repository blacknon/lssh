...
func gcpConnectorConfig(config map[string]interface{}, target providerapi.ConnectorTarget) (gcpIAPConfig, error) {
	cfg := gcpIAPConfig{
		Project:      firstNonEmpty(providerbuiltin.String(target.Config, "project"), providerbuiltin.String(config, "project")),
		Zone:         firstNonEmpty(target.Meta["zone"], providerbuiltin.String(target.Config, "zone"), providerbuiltin.String(config, "zone")),
		InstanceName: firstNonEmpty(target.Meta["name"], providerbuiltin.String(target.Config, "instance_name")),
		Interface:    firstNonEmpty(providerbuiltin.String(target.Config, "interface"), providerbuiltin.String(config, "interface"), "nic0"),
		...
		Credentials:  providerbuiltin.ExpandPath(firstNonEmpty(providerbuiltin.String(target.Config, "credentials_file"), providerbuiltin.String(config, "credentials_file"))),
		Scopes:       firstNonEmptyStringSlice(providerbuiltin.StringSlice(target.Config, "scopes"), providerbuiltin.StringSlice(config, "scopes")),
	}
	if cfg.Project == "" {
...
func gcpConnectorRuntime(config map[string]interface{}, targetConfig map[string]interface{}) string {
	switch strings.ToLower(strings.TrimSpace(firstNonEmpty(
		providerbuiltin.String(targetConfig, "iap_runtime"),
		providerbuiltin.String(config, "iap_runtime"),
	))) {
	case "", "sdk":
		return "sdk"
	case "command":
