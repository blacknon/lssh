...
func azureConnectorConfig(config map[string]interface{}, target providerapi.ConnectorTarget) (azureBastionConnectorConfig, error) {
	cfg := azureBastionConnectorConfig{
		BastionName:      firstNonEmpty(providerbuiltin.String(target.Config, "bastion_name"), providerbuiltin.String(config, "bastion_name")),
		BastionGroup:     firstNonEmpty(providerbuiltin.String(target.Config, "bastion_resource_group"), providerbuiltin.String(config, "bastion_resource_group"), target.Meta["resource_group"]),
		TargetResourceID: firstNonEmpty(target.Meta["id"], providerbuiltin.String(target.Config, "target_resource_id")),
		SubscriptionID:   firstNonEmpty(target.Meta["subscription_id"], providerbuiltin.String(target.Config, "subscription_id"), providerbuiltin.String(config, "subscription_id")),
		...
		Port:             firstNonEmpty(providerbuiltin.String(target.Config, "port"), "22"),
	}
	if cfg.BastionName == "" {
...
func azureConnectorPrepareCommand(params providerapi.ConnectorPrepareParams, cfg azureBastionConnectorConfig) (providerapi.ConnectorPrepareResult, error) {
	...
	if cfg.KeyFile != "" {
		...
		args = append(args, "--auth-type", firstNonEmpty(providerbuiltin.String(params.Target.Config, "bastion_auth_type"), providerbuiltin.String(params.Config, "bastion_auth_type"), "AAD"))
	}
...
func azureConnectorRuntime(config map[string]interface{}, targetConfig map[string]interface{}) string {
	switch strings.ToLower(strings.TrimSpace(firstNonEmpty(
		providerbuiltin.String(targetConfig, "bastion_runtime"),
		providerbuiltin.String(config, "bastion_runtime"),
	))) {
	case "", "sdk":
		return "sdk"
	case "command":
...
func azureTokenScope(config map[string]interface{}) string {
	endpoint := strings.TrimRight(providerbuiltin.String(config, "endpoint"), "/")
	if endpoint == "" {
...
func azureSubscriptionID(ctx context.Context, config map[string]interface{}, baseURL, token string) (string, error) {
	if subscriptionID, err := providerbuiltin.ResolveConfigValue(config, "subscription_id"); err != nil {
		return "", err
