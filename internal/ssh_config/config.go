...
func (ctx *matchContext) applyKV(key, value string) {
...
func (h *Host) IsMatchBlock() bool {
	return h != nil && h.isMatch
}

func (h *Host) matchesContext(ctx matchContext) bool {
	if !h.isMatch {
		return h.Matches(ctx.alias)
	}

	return evaluateMatch(h.matchRaw, ctx)
}

func evaluateMatch(raw string, ctx matchContext) bool {
	...
	for i := 0; i < len(fields); {
		...
		case "host", "originalhost", "user", "localuser":
			...
			for i < len(fields) {
				next := strings.ToLower(fields[i])
				if next == "all" || next == "canonical" || next == "final" || next == "host" || next == "originalhost" || next == "user" || next == "localuser" {
					break
				}
			...
			if !matchCriterionValue(criterion, values, ctx) {
				return false
			}
			matchedAny = true
		default:
...
func matchCriterionValue(criterion string, patterns []string, ctx matchContext) bool {
	var target string
	switch criterion {
	case "host":
		target = ctx.host
	case "originalhost":
		target = ctx.originalHost
	case "user":
		target = ctx.user
	case "localuser":
		target = ctx.localUser
	default:
		return false
	}

	for _, rawPattern := range patterns {
		pattern, err := NewPattern(rawPattern)
		if err != nil {
			continue
		}
