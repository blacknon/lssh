...
func (p *sshParser) parseMatch(val *token, hasEquals bool, comment string) sshParserStateFn {
	...
	case "host", "originalhost", "user", "localuser", "canonical", "final":
		patterns, err := parseMatchPatterns(criterion, fields)
		if err != nil {
			p.raiseErrorf(val, err.Error())
			return nil
		}
		p.config.Hosts = append(p.config.Hosts, &Host{
			Patterns:           patterns,
...
func parseMatchPatterns(criterion string, fields []string) ([]*Pattern, error) {
	switch criterion {
	...
	for _, s := range fields[1:] {
		...
		if lower == "all" || lower == "canonical" || lower == "final" || lower == "host" || lower == "originalhost" || lower == "user" || lower == "localuser" {
			continue
		}
	...
	if len(patterns) == 0 {
		return nil, fmt.Errorf("ssh_config: Match %s requires at least one pattern", fields[0])
	}
