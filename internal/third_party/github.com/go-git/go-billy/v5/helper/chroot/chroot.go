...
func (fs *ChrootHelper) Lchown(path string, uid, gid int) error {
	fullpath, err := fs.underlyingPath(path)
	if err != nil {
		return err
	}

	c, ok := fs.underlying.(billy.Change)
	if !ok {
		return errors.New("underlying fs does not implement billy.Change")
	}
	return c.Lchown(fullpath, uid, gid)
}

func (fs *ChrootHelper) Chown(path string, uid, gid int) error {
	fullpath, err := fs.underlyingPath(path)
	if err != nil {
		return err
	}
	...
	return c.Chown(fullpath, uid, gid)
}

func (fs *ChrootHelper) Chtimes(path string, atime, mtime time.Time) error {
	fullpath, err := fs.underlyingPath(path)
	if err != nil {
		return err
	}
	...
	return c.Chtimes(fullpath, atime, mtime)
}
